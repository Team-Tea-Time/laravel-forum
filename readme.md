...
Deploying the forum controller to your app enables easy overriding of methods.

In **Laravel 4**, this can be done easily using this command:

`php artisan forum:install`

You'll find the controller in your `app/controllers` directory.

In the case of **Laravel 5**, this command is not written to support it (yet), so for L5 you'll need to manually create a controller that extends `\Riari\Forum\Controllers\BaseController`:
...
**Note:** if you want to name the controller anything other than `ForumController`, you'll need to update your config for the package (see below for details about configuration).
