<div class="bg-white px-4 py-3 {{ $thread->pinned ? 'pinned' : '' }} {{ $thread->locked ? 'locked' : '' }} {{ $thread->trashed() ? 'deleted' : '' }}" :class="{ 'border border-blue-500': selectedThreads.includes({{ $thread->id }}) }">
    <div class="flex flex-col md:items-start md:flex-row md:justify-between md:gap-4 p-4">
        ...
        @if ($thread->lastPost)
            <div class="md:w-2/6 text-gray-500 flex flex-col md:flex-row items-center md:justify-end">
